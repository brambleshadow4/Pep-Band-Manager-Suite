...
public class LocationAddDialog extends JDialog {
	...
	public LocationAddDialog(JDialog owner) {
	...
	private void a1Actions() {
		final LocationAddDialog locationAddDialog = this;
		iconAction = new AbstractAction() {
			public void actionPerformed(ActionEvent e) {
				...
				if (iconName == null) {
					...
					iconButton.setIcon(Tools.getLocationIcon(iconName + "32"));
				}
			}
		};
		okAction = new AbstractAction("Add Location") {
			public void actionPerformed(ActionEvent e) {
				DataManager dataManager = DataManager.getDataManager();
				if (nameField.getText().trim().isEmpty()) {
					JOptionPane.showMessageDialog(locationAddDialog, "The name field is empty. Please enter a name for this location.", "Say what?", JOptionPane.ERROR_MESSAGE);
					nameField.requestFocusInWindow();
				} else if (iconChooser.getChoice() == null) {
					JOptionPane.showMessageDialog(locationAddDialog, "You did not select an icon for this location.", "Check yo'self!", JOptionPane.ERROR_MESSAGE);
					iconButton.requestFocusInWindow();
				} else {
					Location newLocation = new Location(dataManager.getDataID(),nameField.getText(),iconChooser.getChoice());
		...
		iconAction.putValue(Action.LONG_DESCRIPTION,"Choose an icon for this location");
		iconAction.putValue(Action.SHORT_DESCRIPTION,"Choose an icon");
		
		okAction.putValue(Action.MNEMONIC_KEY,KeyEvent.VK_A);
		okAction.putValue(Action.LONG_DESCRIPTION,"Add this location to the program's database with the entered properties");
		okAction.putValue(Action.SHORT_DESCRIPTION,"Add new location");
	...
	private void a2Components() {
		northPanel = new GradientPanel("Add a New Location",Tools.getHeaderIcon("paint"));
		
		iconChooser = new IconChooser(this,Tools.getLocationIcons());
