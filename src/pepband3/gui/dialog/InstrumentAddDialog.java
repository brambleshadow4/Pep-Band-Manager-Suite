...
public class InstrumentAddDialog extends JDialog {
	...
	public InstrumentAddDialog(JDialog owner) {
		super(owner,"Add Instrument",true);
	...
	private void a1Actions() {
		final InstrumentAddDialog instrumentAddDialog = this;
		...
		okAction = new AbstractAction("Add Instrument") {
			public void actionPerformed(ActionEvent e) {
				DataManager dataManager = DataManager.getDataManager();
				if (nameField.getText().trim().isEmpty()) {
					JOptionPane.showMessageDialog(instrumentAddDialog, "The name field is empty. Please enter a name for this instrument.", "Say what?", JOptionPane.ERROR_MESSAGE);
					nameField.requestFocusInWindow();
				} else if (iconChooser.getChoice() == null) {
					JOptionPane.showMessageDialog(instrumentAddDialog, "You did not select an icon for this instrument.", "Check yo'self!", JOptionPane.ERROR_MESSAGE);
					iconButton.requestFocusInWindow();
				} else {
					Instrument newInstrument = new Instrument(dataManager.getDataID(),nameField.getText(),iconChooser.getChoice());
					dataManager.addInstrument(newInstrument);
					setVisible(false);
				}
			}
		};
		...
		okAction.putValue(Action.LONG_DESCRIPTION,"Add this instrument to the program's database with the entered properties");
		okAction.putValue(Action.SHORT_DESCRIPTION,"Add new instrument");
	...
	private void a2Components() {
		northPanel = new GradientPanel("Add a New Instrument",Tools.getHeaderIcon("note"));
