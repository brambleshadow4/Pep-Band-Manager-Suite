...
public class LocationEditDialog extends JDialog {
	...
	public LocationEditDialog(JDialog owner) {
		super(owner,"Edit Location",true);
	...
	private void a1Actions() {
		final LocationEditDialog locationEditDialog = this;
		...
		okAction = new AbstractAction("Apply Changes") {
			public void actionPerformed(ActionEvent e) {
				...
				if (nameField.getText().trim().isEmpty()) {
					JOptionPane.showMessageDialog(locationEditDialog, "The name field is empty. Please enter a name for this location.", "Say what?", JOptionPane.ERROR_MESSAGE);
					nameField.requestFocusInWindow();
				} else if (iconChooser.getChoice() == null) {
					JOptionPane.showMessageDialog(locationEditDialog, "No icon is selected for this location.", "Check yo'self!", JOptionPane.ERROR_MESSAGE);
					iconButton.requestFocusInWindow();
				} else {
					location.setName(nameField.getText());
					location.setIconName(iconChooser.getChoice());
					setVisible(false);
				}
			}
		};
		...
		okAction.putValue(Action.LONG_DESCRIPTION,"Apply the changes made to this location");
	...
	private void a2Components() {
		northPanel = new GradientPanel("Edit Selected Location",Tools.getHeaderIcon("worldbox"));
	...
	public void display(Location value) {
		if (value != null) {
			location = value;
			nameField.setText(location.getName());
			iconButton.setIcon(Tools.getLocationIcon(location.getIconName() + "32"));
			iconChooser.setChoice(location.getIconName());
			pack();
